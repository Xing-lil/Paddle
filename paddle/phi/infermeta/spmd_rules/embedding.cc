...
SpmdInfo EmbeddingInferSpmdUnsupportVocabParallel(const DistMetaTensor& x,
  ...
  VLOG(0) << "weight.dist_attr()====" << w.dist_attr();
...
SpmdInfo EmbeddingInferSpmd(const DistMetaTensor& x,
  ...
  // padding_idx s not supported by c_embedding kernel. 检查 padding_idx 和
  // sparse 参数 (TODO) might be could reshard as replicated when padding_idx !=
  // -1
  if (padding_idx != -1) {
    PADDLE_ENFORCE_EQ(
  ...
  if (weight_row_axis_mapping > -1) {
    ...
    VLOG(0) << "### lzx ### EmbeddingInferSpmd:: partial_on_dims"
            << partial_on_dims[0];
  }
  ...
  VLOG(0) << "### lzx ### x_dist_attr_dst," << x_dist_attr_dst << ","
          << weight_dist_attr_dst << "," << out_dist_attr;
...
SpmdInfo EmbeddingGradInferSpmd(const DistMetaTensor& x,
  ...
  VLOG(0) << "in EmbeddingGradInferSpmd";

  DistMetaTensor w_dst(weight.dims(), weight.dist_attr());
  if (weight.dist_attr().dims_mapping()[0] >= 0) {
    VLOG(0) << "in EmbeddingGradInferSpmd w_dst_dims_mapping[0] = -1;";
...
SpmdInfo CEmbeddingGradInferSpmd(const DistMetaTensor& weight,
  ...
  if (x_dst.dist_attr().dims_mapping() !=
      std::vector<int64_t>(t0_dims.begin(), t0_dims.end() - 1)) {
    VLOG(0) << "x_dst.dist_attr().dims_mapping()" << t0.dist_attr();
    TensorDistAttr t0_new(t0.dist_attr());
    t0_new.set_dims_mapping(
        std::vector<int64_t>(t0_dims.begin(), t0_dims.end() - 1));
    x_dst = DistMetaTensor(x_dst.dims(), t0_new);
  ...
  VLOG(0) << "CEmbeddingGradInferSpmd:\n"
          << "Input x shape: [" << str_join(phi::vectorize(x.dims()))
          << "], src_dims_mapping: [" << str_join(x.dist_attr().dims_mapping())
          << "], dst_dims_mapping: ["
          << str_join(x_dst.dist_attr().dims_mapping()) << "]\n"
  ...
  return {{w_dst.dist_attr(), x_dst.dist_attr(), out_grad.dist_attr()},
          //   return {{x_dst.dist_attr(), w_dst.dist_attr(),
